...
<nav class="flex items-center gap-2 mb-xl text-on-surface-variant">
    ...
    <span class="material-symbols-outlined text-sm" data-icon="chevron_right">chevron_right</span>
    <span class="text-body-sm font-bold text-primary">{{ __('messages.common.create') }}</span>
</nav>

<div class="flex justify-between items-end mb-lg">
    <div>
        <h2 class="font-display-xl text-display-xl text-on-surface">{{ __('messages.common.create') }} {{ __('messages.payments.title') }}</h2>
            ...
            <div>
                ...
                <select id="apartment_id" name="apartment_id" required
                    ...
                    @foreach($apartments as $id => $name)
                        <option value="{{ $id }}" {{ old('apartment_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @error('apartment_id')
                    <p class="mt-1 text-body-sm text-error">{{ $message }}</p>
