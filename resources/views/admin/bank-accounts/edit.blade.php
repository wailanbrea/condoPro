...
<nav class="flex items-center gap-2 mb-xl text-on-surface-variant">
    ...
    <span class="material-symbols-outlined text-sm" data-icon="chevron_right">chevron_right</span>
    <span class="text-body-sm font-bold text-primary">Editar Cuenta</span>
</nav>

<div class="flex justify-between items-end mb-lg">
    <div>
        <h2 class="font-display-xl text-display-xl text-on-surface">Editar Cuenta Bancaria</h2>
        <p class="text-on-surface-variant">{{ $bankAccount->bank_name }} — {{ $bankAccount->account_number }}</p>
    </div>
    ...
    <form action="{{ route('bank-accounts.update', $bankAccount) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div>
                ...
                <input type="text" id="account_holder" name="account_holder" value="{{ old('account_holder', $bankAccount->account_holder) }}" required
            ...
            <div>
                ...
                <select id="account_type" name="account_type" required
                    ...
                    <option value="savings" {{ old('account_type', $bankAccount->account_type) === 'savings' ? 'selected' : '' }}>Ahorro</option>
                    <option value="checking" {{ old('account_type', $bankAccount->account_type) === 'checking' ? 'selected' : '' }}>Corriente</option>
                </select>
                @error('account_type')
                    <p class="mt-1 text-body-sm text-error">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="currency" name="currency" required
                    ...
                    <option value="DOP" {{ old('currency', $bankAccount->currency) === 'DOP' ? 'selected' : '' }}>Peso Dominicano (DOP)</option>
                    <option value="USD" {{ old('currency', $bankAccount->currency) === 'USD' ? 'selected' : '' }}>Dólar Estadounidense (USD)</option>
                </select>
                @error('currency')
                    <p class="mt-1 text-body-sm text-error">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="status" name="status"
                    ...
                    <option value="active" {{ old('status', $bankAccount->status) === 'active' ? 'selected' : '' }}>{{ __('messages.common.active') }}</option>
                    <option value="inactive" {{ old('status', $bankAccount->status) === 'inactive' ? 'selected' : '' }}>{{ __('messages.common.inactive') }}</option>
                </select>
                @error('status')
                    <p class="mt-1 text-body-sm text-error">{{ $message }}</p>
                @enderror
