...
<nav class="flex items-center gap-2 mb-xl text-on-surface-variant">
    ...
    <span class="text-body-sm font-bold text-primary">{{ __('messages.apartments.title') }}</span>
</nav>

<div class="flex justify-between items-end mb-lg">
    <div>
        <h2 class="font-display-xl text-display-xl text-on-surface">{{ __('messages.apartments.title') }}</h2>
        <p class="text-on-surface-variant">{{ __('messages.apartments.list') }}</p>
    </div>
    <a href="{{ route('apartments.create') }}" class="px-lg py-2 bg-primary text-white rounded-lg flex items-center gap-2 hover:brightness-110 transition-all">
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-surface-container-low/50">
                <tr>
                    ...
                    <th class="px-lg py-md text-label-caps font-label-caps text-on-surface-variant">{{ __('messages.apartments.balance') }}</th>
                    <th class="px-lg py-md text-label-caps font-label-caps text-on-surface-variant">{{ __('messages.apartments.maintenance_fee') }}</th>
                    <th class="px-lg py-md text-label-caps font-label-caps text-on-surface-variant">{{ __('messages.apartments.has_gas_meter') }}</th>
            ...
            <tbody class="divide-y divide-surface-container-low">
                @forelse($apartments as $apartment)
                    <tr class="hover:bg-surface-container-low transition-colors">
                        ...
                        <td class="px-lg py-md">{{ $apartment->owner_name ?? '—' }}</td>
                        <td class="px-lg py-md text-on-surface-variant">{{ $apartment->condominium->name ?? '—' }}</td>
                        <td class="px-lg py-md font-mono-data text-mono-data">{{ number_format($apartment->balance, 2) }}</td>
                        <td class="px-lg py-md font-mono-data text-mono-data">{{ number_format($apartment->maintenance_fee, 2) }}</td>
                        <td class="px-lg py-md">
                            @if($apartment->has_gas_meter)
                                <span class="material-symbols-outlined text-secondary" data-icon="check_circle">check_circle</span>
                            @else
                                <span class="material-symbols-outlined text-outline" data-icon="cancel">cancel</span>
                            @endif
                        </td>
                        <td class="px-lg py-md">
                        ...
                        <td class="px-lg py-md">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('apartments.edit', $apartment) }}" class="p-1 hover:bg-surface-container-high rounded-lg text-on-surface-variant hover:text-primary transition-colors" title="{{ __('messages.common.edit') }}">
                                    <span class="material-symbols-outlined text-lg" data-icon="edit">edit</span>
                                </a>
                                <form action="{{ route('apartments.destroy', $apartment) }}" method="POST" onsubmit="return confirm('{{ __('messages.common.delete_confirm') }}')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-lg py-xl text-center text-on-surface-variant">{{ __('messages.common.no_results') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
