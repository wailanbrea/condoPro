...
<div class="flex justify-between items-end mb-lg">
    <div>
        ...
        <p class="text-on-surface-variant">{{ app()->getLocale() === 'es' ? 'Centro de notificaciones del condominio' : 'Condominium notification center' }}</p>
    </div>
    <div class="flex items-center gap-md">
        @if($notifications->count() > 0)
        <form action="{{ route('notifications.clearAll') }}" method="POST" onsubmit="return confirm('{{ app()->getLocale() === 'es' ? '¿Eliminar todas las notificaciones?' : 'Delete all notifications?' }}')">
            @csrf
            @method('DELETE')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden {{ $wasUnread ? 'ring-2 ring-primary/20 bg-primary/5' : '' }}">
            <div class="p-lg flex items-start gap-md">
                ...
                <div class="flex-shrink-0 flex items-center gap-sm">
                    <form action="{{ route('notifications.destroy', $notification) }}" method="POST" onsubmit="return confirm('{{ app()->getLocale() === 'es' ? '¿Eliminar esta notificación?' : 'Delete this notification?' }}')">
                        @csrf
                        @method('DELETE')
