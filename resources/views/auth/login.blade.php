...
<head>
    ...
    <title>{{ config('app.name', 'CondoPro') }} - {{ __('messages.auth.login') }}</title>
    ...
    <div class="w-full max-w-md px-6">
        <div class="bg-white rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.08)] p-8 sm:p-10">
            <div class="flex flex-col items-center mb-8">
                ...
                <p class="text-on-surface-variant text-body-md mt-1">{{ __('messages.auth.login_title') }}</p>
            </div>

            @if (session('status'))
                <div class="mb-4 px-4 py-3 bg-secondary-container/20 text-secondary rounded-lg text-body-md">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="flex items-center justify-between mb-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-body-md text-on-surface-variant">{{ __('messages.auth.remember') }}</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-primary-container text-body-md font-bold hover:underline">
            ...
            <div class="mt-6 text-center">
                <p class="text-on-surface-variant text-body-md">
                    ...
                    <a href="{{ route('register') }}" class="text-primary-container font-bold hover:underline">{{ __('messages.auth.register') }}</a>
                </p>
            </div>
        </div>
