...
<nav class="flex items-center gap-2 mb-xl text-on-surface-variant">
    ...
    <span class="material-symbols-outlined text-sm" data-icon="chevron_right">chevron_right</span>
    <span class="text-body-sm font-bold text-primary">{{ $condominium->name }}</span>
</nav>

<div class="flex justify-between items-end mb-lg">
    <div>
        <h2 class="font-display-xl text-display-xl text-on-surface">{{ $condominium->name }}</h2>
        <p class="text-on-surface-variant">{{ __('messages.condominiums.list') }}</p>
    </div>
    <div class="flex gap-md">
        <a href="{{ route('condominiums.edit', $condominium) }}" class="px-lg py-2 bg-primary text-white rounded-lg flex items-center gap-2 hover:brightness-110 transition-all">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-gutter mb-xl">
    <div class="bg-white p-lg rounded-xl shadow-[0px_2px_4px_rgba(0,0,0,0.05)] border-l-4 border-primary">
        <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.name') }}</p>
        <h3 class="font-headline-lg text-headline-lg text-primary">{{ $condominium->name }}</h3>
    </div>
    <div class="bg-white p-lg rounded-xl shadow-[0px_2px_4px_rgba(0,0,0,0.05)] border-l-4 border-secondary">
        ...
        <h3 class="font-headline-lg text-headline-lg text-secondary">{{ $condominium->currency }}</h3>
    </div>
    <div class="bg-white p-lg rounded-xl shadow-[0px_2px_4px_rgba(0,0,0,0.05)] border-l-4 border-outline">
        ...
        @switch($condominium->status)
            ...
            @case('inactive')
                ...
                @break
            @default
                <span class="px-3 py-1 bg-surface-container-low text-on-surface-variant rounded-full text-[11px] font-bold uppercase tracking-wider">{{ $condominium->status }}</span>
        @endswitch
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-[0px_2px_4px_rgba(0,0,0,0.05)] p-lg mb-xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
        <div>
            <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.address') }}</p>
            <p class="text-on-surface">{{ $condominium->address }}</p>
        </div>
        <div>
            <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.phone') }}</p>
            <p class="text-on-surface">{{ $condominium->phone ?? '—' }}</p>
        </div>
        <div>
            <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.email') }}</p>
            <p class="text-on-surface">{{ $condominium->email ?? '—' }}</p>
        </div>
        <div>
            <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.language_default') }}</p>
            <p class="text-on-surface">{{ $condominium->language_default === 'es' ? __('messages.condominiums.spanish') : __('messages.condominiums.english') }}</p>
        </div>
        <div>
            <p class="text-on-surface-variant font-label-caps text-label-caps mb-xs">{{ __('messages.condominiums.logo') }}</p>
            @if($condominium->logo)
                <img src="{{ \Storage::url($condominium->logo) }}" alt="{{ $condominium->name }}" class="h-16 w-auto rounded">
            @else
...
<div class="bg-white rounded-xl shadow-[0px_2px_4px_rgba(0,0,0,0.05)] overflow-hidden mb-xl">
    <div class="p-lg border-b border-surface-container-low">
        <h4 class="font-headline-md text-headline-md">{{ __('messages.condominiums.title') }} — {{ __('messages.apartments.title') }}</h4>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-surface-container-low">
                @forelse($condominium->apartments as $apartment)
                    <tr class="hover:bg-surface-container-low transition-colors">
                        <td class="px-lg py-md font-bold text-on-surface">{{ $apartment->number }}</td>
                        <td class="px-lg py-md text-on-surface-variant">{{ $apartment->owner_name ?? '—' }}</td>
